...
public:
	void sign_pattern_ending() {
		m_pattern_ending = true;
	}

	bool is_pattern_ending() {
		return m_pattern_ending;
	}
...
private:
	bool match_text(const std::string::const_iterator & p_begin, const std::string::const_iterator & p_end) {
		...
		for (std::string::const_iterator begin = p_begin; begin != p_end && !cursor->is_pattern_ending(); begin++) {
		...
		return cursor->is_pattern_ending() ? true : false;
	}
